...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $role === 'admin') {
  ...
  if ($_POST['action'] === 'add') {
    ...
    foreach ($prefixes as $i => $prefix) {
      $prefix    = mysqli_real_escape_string($conn, trim($prefix));
      $warehouse = mysqli_real_escape_string($conn, trim($warehouses[$i] ?? ''));
    ...
    echo $allInserted ? "success" : "error";
    exit;
  }
  ...
  if ($_POST['action'] === 'update') {
    ...
    $warehouse = mysqli_real_escape_string($conn, trim($_POST['warehouse_name'] ?? ''));
    $stock     = intval($_POST['stock'] ?? 0);

    if ($id > 0 && $prefix && $warehouse && $stock >= 0) {
      $sql = "UPDATE warehouse SET prefix='$prefix', warehouse_name='$warehouse', stock=$stock WHERE id=$id";
      ...
      <div class="card shadow-sm rounded-4 mt-4">
        <div class="card-body">
          ...
          <div id="addWarehouseForm" style="display: none;">
            <div class="mb-2 text-start">ADD NEW WAREHOUSE</div>
            <!-- Add Form -->
            <form id="warehouseAddForm" method="POST" action="" class="mb-3">
              ...
              <div class="">
                <button type="submit" class="btn btn-sm btn-success btn-lg mt-1 mx-3" style="width: 70px;">Save</button>
                <a href="warehouse_list.php" class="btn btn-sm btn-secondary btn-lg mt-1 mx-3" style="width: 70px;">Cancel</a>
              </div>
            </form>
          </div>

          <div id="insertMessage" class="my-2 w-75 mx-auto"></div>
          ...
          <div class="table-responsive">
            ...
            <!-- for alert msg -->
            <div id="editMessage" class="my-2 w-75 mx-auto"></div>
